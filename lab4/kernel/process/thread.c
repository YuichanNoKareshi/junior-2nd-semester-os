		       ...
		       const char *bin, struct process_metadata *metadata)
{
	...
	for (i = 0; i < elf->header.e_phnum; ++i) {
		...
		if (elf->p_headers[i].p_type == PT_LOAD) {
			...
			u64 left = p_vaddr - ROUND_DOWN(p_vaddr, PAGE_SIZE);
			seg_map_sz = ROUND_UP(seg_sz + left, PAGE_SIZE);
			...
			memcpy((void*)((phys_to_virt(pmo->start)) + left), bin + elf->p_headers[i].p_offset, elf->p_headers[i].p_filesz);
