...
static u64 thread_migrate_to_server(struct ipc_connection *conn, u64 arg)
{
	 ...
	 * */
	arch_set_thread_stack(target, conn->server_stack_top); //LAB4_IPC_BLANK
	/**
	 * Lab4
	 ...
	 * to the server?
	 * */
	arch_set_thread_next_ip(target, target->server_ipc_config->callback); //LAB4_IPC_BLANK
	/**
	 * Lab4
	 * The argument set by sys_ipc_call;
	 */
	arch_set_thread_arg(target, arg); //LAB4_IPC_BLANK
...
u64 sys_ipc_call(u32 conn_cap, ipc_msg_t * ipc_msg)
{
	 ...
	 * */
	arg = (u64)conn->buf.server_user_addr; //LAB4_IPC_BLANK
