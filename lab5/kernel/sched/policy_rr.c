...
struct thread *rr_sched_choose_thread(void)
{
	...
	struct thread *result = list_entry(rr_ready_queue[cpu_id].next, struct thread, ready_queue_node);
	BUG_ON(rr_sched_dequeue(result));
...
int rr_sched(void)
{
	if (current_thread != NULL && current_thread->thread_ctx->type != TYPE_IDLE && current_thread->thread_ctx->sc->budget != 0)
		return 0;
	struct thread* result = rr_sched_choose_thread();
	BUG_ON(result == NULL);
	...
	result->thread_ctx->sc->budget = DEFAULT_BUDGET;

	return switch_to_thread(result);
}
...
{
	if (current_thread->thread_ctx->type != TYPE_IDLE && current_thread->thread_ctx->sc->budget != 0)
