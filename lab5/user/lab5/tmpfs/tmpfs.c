	...
	while (true)
	{
		...
		else
		{
			...
			if ( (dent = tfs_lookup(*dirat, start, len)) == NULL )
			{
				...
				if (!mkdir_p)
					return -1;
				else if (tfs_mkdir(*dirat, start, 0) != 0)
					return -1;
		       ...
		       size_t size)
{
	...
	void *page;
	inode->size = (offset + size > inode->size) ? offset + size : inode->size; // choose bigger
	if (page_no > page_bound)
	{
		for (u64 i = page_bound; i < page_no; i++)
		{
			page = radix_get(&inode->data, i);
			if (page == NULL)
			{
				page = malloc(PAGE_SIZE);
				radix_add(&inode->data, i, page);
			}
		}
	}

	size_t return_size = size;
	size_t cur_off = 0;
	size_t to_write;

	// TODO: write your code here
	for (u64 i = page_no; i <= page_end; i++)
	{
		page = radix_get(&inode->data, i);
		if (page == NULL)
		{
		...
		if (i == page_no)
		{
			to_write = (size + page_off < PAGE_SIZE) ? size : PAGE_SIZE - page_off; // choose smaller
			size -= to_write;
			memcpy(page + page_off, data + cur_off, to_write);
			cur_off += to_write;
		}
		else
		{
			to_write = size < PAGE_SIZE ? size : PAGE_SIZE ; // choose smaller
			size -= to_write;
			memcpy(page, data + cur_off, to_write);
			cur_off += to_write;
		}
	}
		      ...
		      size_t size)
{
	...
	size = (inode->size < size + offset) ? inode->size - offset : size; // choose smaller 
	size_t return_size = size;
	...
	for (u64 i = page_no; i <= page_end; i++)
	{
		...
		if (i == page_no)
		{
			to_read = (size + page_off < PAGE_SIZE) ? size : PAGE_SIZE - offset; // choose smaller
			size -= to_read;
		...
		{
			to_read = size < PAGE_SIZE ? size : PAGE_SIZE; //choose smaller
			size -= to_read;
			memcpy(buff + cur_off, page, to_read);
			cur_off += to_read;
		}
	}
	return return_size;
...
int tfs_load_image(const char *start)
{
	...
	for (f = g_files.head.next; f; f = f->next) {
		...
		{
			name_len = 0;
			while (*(leaf + name_len) && *(leaf + name_len) != '/')
				name_len ++;
